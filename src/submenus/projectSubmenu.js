...
export async function projectMenu(commandFactory) {
  ...
  while (mainLoop) {
    const { action } = await inquirer.prompt([
      { type: 'select', name: 'action', message: '--- GESTIÓN DE PROYECTOS ---',
        choices: [
      ...
      switch (action) {
        case 'CREATE': {
          const r = await inquirer.prompt([
            { type: 'input', name: 'name', message: 'Nombre del proyecto:', validate: (v) => (v && v.trim() ? true : 'El nombre es obligatorio.') },
            ...
            { type: 'input', name: 'budget', message: 'Presupuesto inicial:', validate: validarMonto },
            ...preguntasFecha('inicio del proyecto', 'inicio'),
            ...preguntasFecha('fin estimado del proyecto', 'fin'),
          ]);
          const data = {
            name: r.name, clientId: r.clientId, budget: Number(r.budget),
        ...
        case 'LIST': {
          ...
          proyectos.forEach((p) => table.push([(p.id ?? p._id).toString(), p.name, p.clientId.toString(), formatMoneda(p.budget), formatEstado(p.status), formatFecha(p.startDate)]));
        ...
        case 'BY_CLIENT': {
          ...
          proyectos.forEach((p) => table.push([(p.id ?? p._id).toString(), p.name, formatMoneda(p.budget), formatEstado(p.status), formatFecha(p.startDate)]));
          console.log(table.toString());
          break;
        }
        case 'UPDATE_STATUS': {
          const { id, status } = await inquirer.prompt([
            ...
            { type: 'select', name: 'status', message: 'Seleccione el nuevo estado:', choices: ['Planificado', 'En progreso', 'En espera', 'Completado', 'Cancelado'] },
          ]);
