...
export const PROJECT_ID_REGEX = /^[0-9]+$/;
export const ESTADOS_CONTRATO = ['borrador', 'activo', 'finalizado', 'cancelado'];

class Contract extends BaseModel {
  constructor({ projectId, clientId, fechaInicio, fechaFin, valorTotal, condiciones, status = 'borrador' } = {}) {
    super();
  ...
  validate() {
    if (!this.projectId || !PROJECT_ID_REGEX.test(String(this.projectId).trim())) {
      throw new ValidationError('El projectId debe ser un número entero positivo.');
    }
